...
export async function fetchTaxesAndFeesData(): Promise<TaxAndFeesData> {
  ...
  let tcmmvValue: number | null = null;
  let validFrom = "";
  let validTo = "";

  try {
    if (!apiUrl) {
      throw new Error("Missing EXCHANGE_RATE_API_URL environment variable.");
    }

    const res = await fetch(apiUrl, { cache: "no-store" });
    if (!res.ok) {
      throw new Error(`Failed to fetch exchange rates: ${await res.text()}`);
    }
    ...
    tcmmvValue = Math.max(dolar, euro);
    ...
    validFrom = formatDate(mondayDate);
    validTo = formatDate(fridayDate);
  ...
  for (let i = 1; i <= 12; i++) {
    ...
    const totalBs = tcmmvValue !== null ? totalTcmmv * tcmmvValue : null;
